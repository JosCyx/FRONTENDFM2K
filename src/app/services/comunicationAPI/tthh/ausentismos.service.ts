...
export class AusentismosService {
  ...
  constructor(
   ...
   this.globalService.getConfigLoadedObservable().subscribe(
     ...
     getAusentismo(opSelected:string,fechaInicio: Date, fechaFin: Date, usuario: string | null= null): Observable<any> {
         console.log(this.APIUrl);
         const headers = this.getHeadersWithAuthToken();
         const fechaInicioString = fechaInicio.toISOString().split('T')[0];
         const fechaFinString = fechaFin.toISOString().split('T')[0];
        return this.http.get(`${this.APIUrl}/Ausentismos/GetAusentismo?op=${opSelected}&fechaInicio=${fechaInicioString}&fechaFin=${fechaFinString}&solcitante=${usuario}`, {headers: headers });
     }

      getOpList(): Observable<any> {
        console.log(this.APIUrl);
        const headers = this.getHeadersWithAuthToken();  
        return this.http.get(`${this.APIUrl}/AusOperaciones`, {headers: headers })
      }
