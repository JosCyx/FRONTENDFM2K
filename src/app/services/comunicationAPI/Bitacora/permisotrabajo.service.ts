...
export class PermisotrabajoService {
  ...
  postpermtrabajo(data: any) {
    const headers = this.getHeadersWithAuthToken();
    return this.http.post(this.APIUrl + '/BtcPermTrabajo', data, { headers: headers });
  }
  getpermtrabajo():Observable<any>{
    
    const headers = this.getHeadersWithAuthToken();
    return this.http.get<any>(this.APIUrl + '/BtcPermTrabajo', { headers: headers });
  }
}
